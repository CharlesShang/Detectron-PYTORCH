...
def sample_rpn_outputs_wrt_gt_boxes(boxes, scores, gt_boxes, is_training=False, only_positive=False):
    ...
    if gt_boxes.size > 0:
        ...
        fg_inds = np.where(max_overlaps >= cfg.fg_threshold)[0]

        mask_fg_inds = np.where(max_overlaps >= cfg.mask_threshold)[0]
        if mask_fg_inds.size > cfg.masks_per_image:
            mask_fg_inds = np.random.choice(mask_fg_inds, size=cfg.masks_per_image, replace=False)

        if True:
            gt_argmax_overlaps = overlaps.argmax(axis=0) # G
            fg_inds = np.union1d(gt_argmax_overlaps, fg_inds)

        fg_rois = int(min(fg_inds.size, cfg.rois_per_image * cfg.roi_fg_fraction))
        ...
        bg_inds = np.where((max_overlaps < cfg.rpn_bg_threshold))[0]
        bg_rois = max(min(cfg.rois_per_image - fg_rois, fg_rois * 3), 64)
        ...
        bg_rois = min(int(cfg.rois_per_image * (1-cfg.roi_fg_fraction)), 64)
