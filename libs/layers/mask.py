...
def encode(gt_masks, gt_boxes, rois, num_classes, mask_height, mask_width):
  ...
  if gt_boxes.size > 0: 
      ...
      keep_inds = np.where(max_overlaps >= cfg.mask_threshold)[0]
      num_masks = int(min(keep_inds.size, cfg.masks_per_image))
      ...
      ignore_inds = np.where((max_overlaps < cfg.fg_threshold))[0]
