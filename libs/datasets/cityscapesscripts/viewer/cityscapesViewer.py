...
from labels import labels, name2label, assureSingleInstanceName
...
class CityscapesViewer(QtWidgets.QMainWindow):
    ...
    def selectImage(self):
        ...
        items = QStringList([os.path.basename(i) for i in self.images])
    ...
    def getCityFromUser(self):
        ...
        if items:
            ...
            (item, ok) = QtWidgets.QInputDialog.getItem(self, dlgTitle, question, items, 0, False)
            # Restore message
            self.statusBar().showMessage(restoreMessage)
            if ok and item:
