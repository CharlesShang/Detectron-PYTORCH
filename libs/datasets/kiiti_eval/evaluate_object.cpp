...
bool my_eval(string gt_dir, string result_dir, string valtxt_path, Mail* mail){
  ...
  ifstream val_set_file(valtxt_path.c_str());
  string line;
  vector<string> val_set;
  if(val_set_file.is_open()){
    while (getline(val_set_file, line)){
      val_set.push_back(line);
    }
    val_set_file.close();
  }
  ...
  // for all images read groundtruth and detections
  mail->msg("Loading detections...");
  for (int32_t i=0; i<val_set.size(); i++) {

    // file name
    // char file_name[256];
    // sprintf(file_name,"%06d.txt",i);
    string file_name = val_set[i] + ".txt";
    cout << file_name <<" " <<gt_dir + "/" + file_name << " " << result_dir + "/" + file_name << endl;
    // read ground truth and result poses
    bool gt_success,det_success;
    vector<tGroundtruth> gt   = loadGroundtruth(gt_dir + "/" + file_name, gt_success);
    ...
    // check for errors
    if (!gt_success) {
      mail->msg("ERROR: Couldn't read: %s of ground truth. Please write me an email!", file_name.c_str());
      return false;
    }
    if (!det_success) {
      mail->msg("ERROR: Couldn't read: %s of detections", file_name.c_str());
      return false;
    }
  }
  mail->msg("  done.");
...
int32_t main (int32_t argc,char *argv[]) {
  ...
  if (argc!=2 && argc!=4) {
    cout << "Usage: ./eval_detection gt_dir result_dir valtxt_path" << endl;
    return 1;
  }
  ...
  // run evaluation
  if (my_eval(gt_dir, result_dir, valtxt_path, mail)) {
